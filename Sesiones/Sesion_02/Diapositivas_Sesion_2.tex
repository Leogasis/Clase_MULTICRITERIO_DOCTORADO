...
\begin{frame}{Ponderación de Criterios en Decisiones Multicriterio}
    \begin{itemize}
        \item La importancia relativa o peso ($w_j$) de los criterios ($C_j$) es un elemento central de los modelos MADM/MCDM.
...
% Slide 4b: Posicionamiento de Métodos: Compensatorios vs. No Compensatorios
\begin{frame}{Posicionamiento de Métodos: Compensatorios vs. No Compensatorios}
    En la teoría de decisiones multicriterio (MCDM), los métodos se estructuran según su flexibilidad de compensación:
    \begin{columns}[T]
        \begin{column}{0.48\textwidth}
            \begin{block}{Métodos Compensatorios}
                \scriptsize
                Un desempeño deficiente en un criterio puede ser contrarrestado (compensado) por un desempeño excelente en otro.
                \begin{itemize}
                    \item \textbf{Filosofía:} Intercambio mutuo (\textit{trade-off}) de valores.
                    \item \textbf{AHP y BWM:} Determinan pesos para esquemas aditivos compensatorios.
                    \item \textbf{Ejemplos:} AHP, BWM, TOPSIS, VIKOR, MAUT.
                \end{itemize}
            \end{block}
        \end{column}
        
        \begin{column}{0.48\textwidth}
            \begin{exampleblock}{Métodos No Compensatorios}
                \scriptsize
                No se permite la compensación. Si una alternativa reprueba un criterio crítico, es descartada o vetada de inmediato.
                \begin{itemize}
                    \item \textbf{Filosofía:} Reglas estrictas de dominancia y veto.
                    \item \textbf{Ejemplo logístico:} Un transportista sin seguro de carga es vetado sin importar que tenga la tarifa más baja.
                    \item \textbf{Ejemplos:} ELECTRE, PROMETHEE.
                \end{itemize}
            \end{exampleblock}
        \end{column}
    \end{columns}
...
\begin{frame}{La Escala Fundamental de Comparación}
    Saaty propuso una escala de medición discreta basada en estudios de psicología cognitiva (Ley de Miller, $7 \pm 2$):
    \begin{center}
        \scriptsize
...
\begin{frame}{Construcción de la Matriz de Comparaciones Pareadas}
    ...
    \begin{itemize}
        ...
        \item El número de comparaciones pareadas requeridas para $n$ criterios es:
              $N = \frac{n(n-1)}{2}$
        \item Para $n=10$, ¡se requieren 45 comparaciones! Alto riesgo de fatiga en el decisor.
    \end{itemize}
\end{frame}

% Slide 8b: Ejemplo de Matriz AHP
\begin{frame}{Ejemplo: Matriz de Comparaciones Pareadas AHP}
    Para la selección de un transportista LTL con 4 criterios: Costo ($C_1$), Confiabilidad ($C_2$), Visibilidad ($C_3$) y Sustentabilidad ($C_4$).
    El decisor genera la siguiente matriz recíproca positiva $\mathbf{A} \in \mathbb{R}^{4 \times 4}$:
    \[\mathbf{A} = \begin{pmatrix}
        1 & 1/2 & 2 & 3 \\
        2 & 1 & 3 & 6 \\
        1/2 & 1/3 & 1 & 2 \\
    ...
    \end{pmatrix}\]
    \begin{itemize}
        \item El número de comparaciones realizadas es $N = \frac{4(3)}{2} = 6$.
        \item Nota que se cumple la reciprocidad: $a_{21} = 2 \implies a_{12} = 1/2$.
    \end{itemize}
\end{frame}

% Slide 9: Vector de Prioridades y el Autovector Principal
\begin{frame}{Elicitación del Vector de Prioridades}
    \small
    ...
    \begin{block}{Método del Autovector Principal (Principal Eigenvector Method)}
        \scriptsize
        \begin{itemize}
            \item Se calcula el vector propio $\mathbf{w}$ correspondiente a $\lambda_{\max}$ de $\mathbf{A}$.
            \item El Teorema de Perron-Frobenius garantiza un único autovector positivo para una matriz positiva.
            \item Se cumple que $\lambda_{\max} \ge n$ (igualdad si y solo si los juicios son perfectamente consistentes).
        \end{itemize}
    \end{block}
\end{frame}

% Slide 9b: Ejemplo de Cálculo del Vector de Prioridades
\begin{frame}{Ejemplo: Elicitación del Vector de Prioridades}
    Utilizando la matriz $\mathbf{A}$ del ejemplo de transportista LTL:
    \begin{enumerate}
        \item \textbf{Suma de columnas de $\mathbf{A}$:}
              $S_{col} = (3.8333, \ 2.0000, \ 6.5000, \ 12.0000)$
        \item \textbf{Normalización por columnas y promedio por fila:}
              \scriptsize
    ...
    \end{enumerate}
    \begin{block}{Vector de Pesos Resultante}
...
\begin{frame}{El Índice y la Razón de Consistencia}
    ...
    \end{columns}
    \smallskip
    Donde $RI$ es el \textbf{Índice Aleatorio}:
    \begin{center}
        \scriptsize
...
% Slide 10b: Ejemplo de Razón de Consistencia en AHP
\begin{frame}{Ejemplo: Cálculo de Consistencia en AHP}
    Para validar los pesos obtenidos $\mathbf{w} \approx (0.2672, \ 0.4958, \ 0.1544, \ 0.0826)^T$:
    \begin{enumerate}
        \item \textbf{Cálculo del vector propio estimado:} $\mathbf{A}\mathbf{w} \approx (1.0717, \ 1.9890, \ 0.6185, \ 0.3315)^T$
        \item \textbf{Estimación del autovalor máximo $\lambda_{\max}$:}
              \[ \lambda_i = \frac{(\mathbf{A}\mathbf{w})_i}{w_i} \implies \lambda_1 \approx 4.0109, \ \lambda_2 \approx 4.0117, \ \lambda_3 \approx 4.0058, \ \lambda_4 \approx 4.0133 \]
              \[ \lambda_{\max} = \text{promedio}(\lambda_i) \approx \mathbf{4.0104} \]
        \item \textbf{Índice de Consistencia ($CI$):} $CI = \frac{4.0104 - 4}{4 - 1} \approx \mathbf{0.0035}$
        \item \textbf{Razón de Consistencia ($CR$):} Con $n=4 \implies RI = 0.90$:
              \[ CR = \frac{0.0035}{0.90} \approx 0.0039 \implies \mathbf{0.39\%} \]
    \end{enumerate}
    \begin{alertblock}{Conclusión de Consistencia}
        ...
        \begin{column}{0.58\textwidth}
            \begin{itemize}
                ...
                \item En lugar de construir una matriz completa, el decisor solo compara:
                      ...
                      \end{enumerate}
                \item Requiere únicamente: $N_{BWM} = 2n - 3$ comparaciones.
            \end{itemize}
        \end{column}
        \begin{column}{0.38\textwidth}
            \begin{exampleblock}{Ventajas de BWM}
                ...
                \begin{itemize}
                    ...
                    \item Formulación matemática lineal y convexa que garantiza una solución única global.
                \end{itemize}
            \end{exampleblock}
        \end{column}
    \end{columns}
...
\begin{frame}{Algoritmo de BWM: Comparaciones Estructuradas}
    \begin{enumerate}
        ...
        \item \textbf{Paso 3:} Elicitar el vector de comparación del mejor criterio respecto a los demás (\textbf{Best-to-Others}, escala 1-9):
              $A_B = (a_{B1}, a_{B2}, \dots, a_{Bn})$ *(Donde $a_{BB} = 1$)*
        \item \textbf{Paso 4:} Elicitar el vector de comparación de todos los criterios respecto al peor (\textbf{Others-to-Worst}, escala 1-9):
              $A_W = (a_{1W}, a_{2W}, \dots, a_{nW})^T$ *(Donde $a_{WW} = 1$)*
    \end{enumerate}
\end{frame}

% Slide 15: Formulación Matemática Minimax de BWM
\begin{frame}{Formulación del Modelo Minimax No Lineal}
    \small
    El objetivo es determinar los pesos óptimos $\mathbf{w}^*$ que minimicen la desviación absoluta máxima:
    \begin{columns}[T]
        \begin{column}{0.48\textwidth}
            \[
            \begin{aligned}
        ...
        \begin{column}{0.48\textwidth}
            \begin{alertblock}{Propiedades del Modelo}
                \scriptsize
                \begin{itemize}
                    \item Este problema no es lineal y no es convexo.
                    \item Rezaei (2016) reformuló el problema de manera lineal para garantizar una solución única global.
                \end{itemize}
            \end{alertblock}
        \end{column}
    \end{columns}
...
% Slide 15b: Ejemplo de Modelo Minimax No Lineal BWM
\begin{frame}{Ejemplo: Modelo Minimax No Lineal BWM}
    Usando los criterios logísticos con Confiabilidad ($C_2$) como el mejor (Best) y Sustentabilidad ($C_4$) como el peor (Worst):
    \begin{columns}[T]
        \begin{column}{0.58\textwidth}
            \scriptsize
            \begin{itemize}
                \item \textbf{Best-to-Others ($A_B$):} $a_{21}=2$, $a_{22}=1$, $a_{23}=3$, $a_{24}=6$
                \item \textbf{Others-to-Worst ($A_W$):} $a_{14}=3$, $a_{24}=6$, $a_{34}=2$, $a_{44}=1$
            \end{itemize}
            El problema de optimización se plantea como:
            \[
            \begin{aligned}
                \min \quad & \xi \\
                \text{s.t.} \quad & \left| \frac{w_2}{w_1} - 2 \right| \le \xi, \quad \left| \frac{w_2}{w_3} - 3 \right| \le \xi, \quad \left| \frac{w_2}{w_4} - 6 \right| \le \xi \\
                & \left| \frac{w_1}{w_4} - 3 \right| \le \xi, \quad \left| \frac{w_3}{w_4} - 2 \right| \le \xi \\
                & w_1 + w_2 + w_3 + w_4 = 1, \quad w_j \ge 0
            \end{aligned}
            \]
        \end{column}
        \begin{column}{0.38\textwidth}
...
\begin{frame}{Formulación Matemática del BWM Linealizado}
    \small
    Rezaei transformó el modelo minimax en un \textbf{Problema de Programación Lineal (PPL)} convexo:
...
% Slide 16b: Ejemplo de Modelo BWM Linealizado
\begin{frame}{Ejemplo: Modelo BWM Linealizado y Pesos Óptimos}
    ...
    \begin{columns}[T]
        \begin{column}{0.52\textwidth}
            ...
            \begin{aligned}
                & \min \quad \xi^L \\
                & \text{s.t.} \\
                & |w_2 - 2w_1| \le \xi^L, \quad |w_2 - 3w_3| \le \xi^L, \quad |w_2 - 6w_4| \le \xi^L \\
                & |w_1 - 3w_4| \le \xi^L, \quad |w_3 - 2w_4| \le \xi^L \\
                & w_1 + w_2 + w_3 + w_4 = 1, \quad w_j \ge 0
            \end{aligned}
            \]
        \end{column}
        \begin{column}{0.44\textwidth}
            \begin{block}{Solución Óptima}
                ...
                \begin{itemize}
                    \item $w_1^* \text{ (Costo)} = \mathbf{0.2500}$
                    \item $w_2^* \text{ (Confiabilidad)} = \mathbf{0.5000}$
                    \item $w_3^* \text{ (Visibilidad)} = \mathbf{0.1667}$
                    \item $w_4^* \text{ (Sustentabilidad)} = \mathbf{0.0833}$
                    \item Desviación óptima: $\xi^{L*} = \mathbf{0.0000}$
                \end{itemize}
            \end{block}
        \end{column}
    \end{columns}
...
\begin{frame}{Medición del Grado de Consistencia en BWM}
    \small
    El valor óptimo $\xi^*$ (o $\xi^L$) indica el nivel de inconsistencia de los juicios. La \textbf{Razón de Consistencia (CR)} es:
    \[ CR = \frac{\xi^*}{CI} \]
    Donde $CI$ es la Consistencia Mínima (Rezaei, 2015):
    \begin{center}
        \scriptsize
        \begin{tabular}{cccccccccc}
            \toprule
    ...
    \end{center}
    \begin{itemize}
        \item A mayor valor de $a_{BW}$ (brecha entre mejor y peor), mayor es el error admisible $\xi^*$.
        \item Si $CR$ es cercano a 0 indica alta consistencia; si $CR > 0.10$ requiere revisión.
    \end{itemize}
\end{frame}

% Slide 17b: Ejemplo de Razón de Consistencia en BWM
\begin{frame}{Ejemplo: Cálculo de Consistencia en BWM}
    Evaluemos la consistencia del decisor en nuestro ejemplo de transportista LTL:
    \begin{itemize}
        \item El valor óptimo de desviación obtenido al resolver el modelo lineal es $\xi^{L*} = \mathbf{0.0000}$.
        \item La preferencia del mejor criterio ($C_2$) frente al peor ($C_4$) es $a_{BW} = a_{24} = \mathbf{6}$.
        \item De la tabla de Consistencia Mínima, para $a_{BW} = 6$, el valor del índice es $CI = \mathbf{3.00}$.
    \end{itemize}
    \begin{block}{Cálculo de la Razón de Consistencia (CR)}
        \[ CR = \frac{\xi^*}{CI} = \frac{0.0000}{3.00} = \mathbf{0.0000} \implies \mathbf{0.0\%} \]
    \end{block}
    \begin{alertblock}{Conclusión de Consistencia en BWM}
...
\begin{frame}{Comparativa Metodológica para Proyectos Doctorales}
    \begin{center}
        ...
        \begin{tabular}{p{2.4cm}p{4.6cm}p{4.6cm}}
            ...
            \textbf{Consistencia} & Propensa a errores de transitividad & Robusta debido a consistencia lógica \\
            ...
            \textbf{Escala} & Recíproca lineal (1-9) & Recíproca lineal (1-9) \\
            \bottomrule
        \end{tabular}
    \end{center}
    ...
    \begin{columns}[T]
        \begin{column}{0.52\textwidth}
            ...
            \begin{itemize}
                ...
                \item \textbf{Confiabilidad ($C_2$):} Exactitud de entrega (OTIF).
                \item \textbf{Visibilidad ($C_3$):} Rastreo GPS en tiempo real.
                \item \textbf{Sustentabilidad ($C_4$):} Emisiones y flota ecológica.
            \end{itemize}
        \end{column}
